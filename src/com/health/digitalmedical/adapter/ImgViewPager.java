...
import com.health.digitalmedical.view.FixedSpeedScroller;
...
public class ImgViewPager extends ViewPager
{
			...
			int normalId)
	{
		...
		{
			this.setCurrentItem((Integer.MAX_VALUE / 2) - (Integer.MAX_VALUE / 2)
					% mListViews.size());// 设置选中为中间/图片为和第0张一样
		}
	}
		...
		timer.schedule(new TimerTask()
		{
			public void run()
			{
				mActivity.runOnUiThread(new Runnable()
				{
					public void run()
					{
						ImgViewPager.this.setCurrentItem(ImgViewPager.this.getCurrentItem() + 1);
					}
				});
			}
